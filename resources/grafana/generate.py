...
D["requests"] = dashboard("cbox-tel-requests", "Telemetry / Requests", [
    ...
    timeseries("Responses by status code", [target(f'sum by (http_response_status_code) (rate({REQ}_count{{{RF}}}[$__rate_interval])) * 60', '{{http_response_status_code}}')],
               ...
               description="4xx is client noise worth watching — auth failures, validation storms, broken API consumers."),
    traces("Client errors (4xx) — click to inspect what the caller sent",
...
D["jobs"] = dashboard("cbox-tel-jobs", "Telemetry / Jobs", [
    ...
    timeseries("Worker memory — a climbing line IS the leak", [
        target(f'histogram_quantile(0.95, sum by (le, queue) (rate(worker_memory_rss_bytes_bucket{{{SVC}}}[$__rate_interval])))', 'rss p95 {{queue}}'),
        target(f'histogram_quantile(0.95, sum by (le, queue) (rate(worker_memory_php_bytes_bucket{{{SVC}}}[$__rate_interval])))', 'php p95 {{queue}}'),
...
D["system"] = dashboard("cbox-tel-system", "Telemetry / System", [
    ...
    row("Workers", 21),
    timeseries("Worker leak curves — p95 RSS after each job, by queue", [target(f'histogram_quantile(0.95, sum by (le, queue) (rate(worker_memory_rss_bytes_bucket{{{SVC}}}[$__rate_interval])))', '{{queue}}')], 0, 22, w=24, unit="bytes",
