...
- Browser/RUM (optional): `TELEMETRY_INGEST_SPANS` opens a bounded, no-token span ingest so the frontend joins the backend trace. `TELEMETRY_SOURCEMAPS` + `TELEMETRY_SOURCEMAPS_TOKEN` open a token-gated source-map upload endpoint (CI can hold the secret); `Support\Symbolicator::symbolicateStack($release, $stack)` resolves minified browser stacks to original source at read time.
- Analytics (optional, `TELEMETRY_ANALYTICS`, default off): shared `session.id` (server span + RUM SDK) so a whole visit is one key; unsampled `analytics.page_view` events (OTLP logs, never undercounted) + browser SPA views/engagement/`track()`; optional built-in geo (`TELEMETRY_ANALYTICS_GEO`, MaxMind suggest) + UA parse (`TELEMETRY_ANALYTICS_UA`). Override via `Telemetry::resolveSessionUsing()` / `resolveClientGeoUsing()` (Cloudflare-friendly). Analytics = logs not spans (sampling-immune). Zero diff when off.
