...
- NativePHP (mobile v4 / desktop v2): `TELEMETRY_STORE=sqlite` + `TELEMETRY_OTLP_SPOOL_DRIVER=sqlite` (no Redis/APCu on a device; the sqlite spool survives an app kill), Prometheus off (nothing can scrape a phone), OTLP push only. State reset on `Native\Mobile\Runtime::onReset()` is automatic. Mobile screens (`instrument.native_screens`, default on but arms nothing by itself) need opt-in wiring: a shared `NativeComponent` base class forwarding `runLoop()`/`dispatch()`/`dispatchNativeEvent()` to `NativeScreenInstrumentation::aroundScreen()`/`aroundInteraction()` — signals `screen.views`, `screen.view.duration`, `screen.interaction.duration{screen,type}`, spans `screen.interaction`/`screen.native_event` with `screen.name`+`screen.event.type`, because a native screen holds one request open for its whole lifetime so `Kernel::terminate()` never fires. Don't instrument `mount()`/`unmount()` — an app's own override silently replaces them. See `docs/cookbook/nativephp.md`. Treat the device as a public client: OTLP tokens in a bundle are extractable, so ship through an ingest gateway.
