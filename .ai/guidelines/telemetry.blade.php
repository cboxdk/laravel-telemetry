...
- Native runtime (optional, `pie install cboxdk/telemetry-native`, config `telemetry.native`): the `cbox_telemetry` extension measures what PHP cannot measure about itself. Requests/jobs/commands/scheduled tasks are bracketed as native units of work, giving a CPU profile with `profile.confidence` (dropped samples + kernel timer overruns reported, not averaged away), exact `pdo.connect`/`redis.connect`/`curl.exec` timing as span attributes + `runtime.operations`/`runtime.operation.duration` metrics, `php.gc.runs`/`php.gc.collected`, and crash records drained by `telemetry:flush` (or `telemetry:crashes`) into FATAL `crash.recorded` events in the trace the process died in. It replaces ext-excimer where both exist. Units do NOT nest — the outermost wins, and `queue:work`/`horizon*`/`octane:*`/`schedule:run` open none so the jobs and tasks inside them can. Set `cbox_telemetry.auto=1` for FPM only (it covers the bootstrap); in a worker it would open one unit per process. Test either path with `Testing\FakeNativeRuntime`. See `docs/production/native-runtime.md`.
