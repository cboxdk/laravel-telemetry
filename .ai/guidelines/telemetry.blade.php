...
- Analytics (optional, `TELEMETRY_ANALYTICS`, default off): stamps a shared `session.id` on server spans and propagates it to the RUM SDK, so a whole visit is one key. Cookieless salted-hash default; override with `Telemetry::resolveSessionUsing($request)` (e.g. Cloudflare `CF-Ray`/cookie) and `Telemetry::resolveClientGeoUsing($request)` (e.g. `CF-IPCountry`). Zero diff when off.
