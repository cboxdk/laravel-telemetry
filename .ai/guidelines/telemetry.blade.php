...
- Outbound HTTP to a host the USER supplied (OAuth issuer, customer webhook, tenant API)? Register `Telemetry::classifyHttpHostsUsing(fn (string $host) => ...)` — `server.address` is a metric label, so each distinct hostname is a permanent series. Return a bounded group, or null to drop the metrics and keep the span.
- Set ambient facets once per request (middleware, after tenant/auth resolution): `Telemetry::context(['team.id' => $id, 'plan' => $plan])` — they flow to every span/event/log AND into dispatched jobs automatically. A `null` value means not set: it removes the dimension, so `['tenant.id' => $tenant?->id]` needs no filtering. Never put unbounded ids in metric labels; context is for traces/events/logs.
...
- CPU profiling: `instrument.profiling` (default on) auto-activates with a profiler extension installed — a `profile.captured` event (top functions by sample count) for requests/jobs slower than `profiling.min_duration_ms`. No dependency, no config needed beyond installing the extension.
- Native runtime (optional, `pie install cboxdk/telemetry-native`, config `telemetry.native`): the `cbox_telemetry` extension measures what PHP cannot measure about itself. Requests/jobs/commands/scheduled tasks are bracketed as native units of work, giving a CPU profile with `profile.confidence` (dropped samples + kernel timer overruns reported, not averaged away), exact `pdo.connect`/`redis.connect`/`curl.exec` timing as span attributes + `runtime.operations`/`runtime.operation.duration` metrics, `php.gc.runs`/`php.gc.collected`, and crash records drained by `telemetry:flush` (or `telemetry:crashes`) into FATAL `crash.recorded` events carrying the trace the process died in (an OTLP log record, not a span — schedule `telemetry:crashes` per host, since sinks are per machine and per uid; nothing drains while telemetry is disabled). It replaces ext-excimer where both exist. Units do NOT nest — the outermost wins, and `queue:work`/`horizon*`/`octane:*`/`schedule:run` open none so the jobs and tasks inside them can. Set `cbox_telemetry.auto=1` for FPM only (it covers the bootstrap); in a worker it would open one unit per process. Test either path with `Testing\FakeNativeRuntime`. See `docs/production/native-runtime.md`.
...
- `telemetry:flush` answers for delivery: a batch the endpoint rejected is printed with exporter + HTTP status + response body, logged at `error`, and exits **non-zero** (partial delivery reads "2 of 3 exporters accepted the batch"). Monitor the scheduled job's exit code. `Telemetry::flush()`/`flushMetrics()` return an `ExportReport` (`successful()`, `summary()`, `problems()`) — they still never throw.
...
- Exceptions: every `report()`ed exception emits a structured OTLP record (`exception.type/message/file/line/stacktrace`, `exception.group` fingerprint, `user.id` for authenticated users, ambient context) for backend error tracking; `exceptions.reported{exception}` counts by class. Fingerprint groups by throw site, not class.
...
- Livewire (`instrument.livewire`, default on, auto-activates when `livewire/livewire` is installed): `livewire.components.mounted`/`.hydrated` counters (mount/hydrate have no "after" phase in Livewire's `ComponentHook` API, so they're counted not timed); `livewire.render`/`.update`/`.call` detail spans for render/property-update/method-call (these DO wrap real work) carrying `livewire.component` + `livewire.property`/`.method`. Livewire update requests are auto-named after their component — `livewire:{component}` (`livewire:batch` for multi-component batches) becomes the root span name and `http.route` label, with the component list as `livewire.components` on the root span.
...
- NativePHP (mobile v4 / desktop v2): `TELEMETRY_STORE=sqlite` + `TELEMETRY_OTLP_SPOOL_DRIVER=sqlite` (no Redis/APCu on a device; the sqlite spool survives an app kill), Prometheus off (nothing can scrape a phone), OTLP push only. State reset on `Native\Mobile\Runtime::onReset()` is automatic. Mobile screens (`instrument.native_screens`, default on) come in two halves: `screen.views`/`screen.view.duration` + `screen.view` events are automatic off the upstream `Native\Mobile\Events\Screen\*` lifecycle events on nativephp/mobile ^4.1 (class_exists-guarded, so 4.0.x never arms them); `screen.interaction`/`screen.native_event` spans need a shared `NativeComponent` base class forwarding `dispatch()`/`dispatchNativeEvent()` to `NativeScreenInstrumentation::aroundInteraction()`, because upstream announces no interaction and that is where the flush must happen — a native screen holds one request open for its whole lifetime so `Kernel::terminate()` never fires. Don't forward `mount()`/`unmount()` — an app's own override silently replaces them. See `docs/cookbook/nativephp.md`. Treat the device as a public client: OTLP tokens in a bundle are extractable, so ship through an ingest gateway.
