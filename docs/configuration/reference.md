...
| `instrument.native_screens` | `TELEMETRY_INSTRUMENT_NATIVE_SCREENS` | `true` — NativePHP for Mobile v4 screens: `screen.interaction`/`screen.native_event` spans carrying `screen.name` + `screen.event.type`, plus `screen.views` counter, `screen.view.duration` and `screen.interaction.duration{screen,type}` histograms. Unlike every other toggle here this one arms nothing by itself — v4 dispatches no screen lifecycle events, so the app opts in by forwarding `runLoop()`/`dispatch()`/`dispatchNativeEvent()` from a `NativeComponent` base class to `NativeScreenInstrumentation` (see the [NativePHP cookbook](../cookbook/nativephp.md)). Turning this off makes those forwards pass straight through, so a consent prompt answered mid-session takes effect without a restart |
