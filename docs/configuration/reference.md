...
| `analytics.geo.enabled` | `TELEMETRY_ANALYTICS_GEO` | `false` — resolve `geo.country_iso_code` at collection time. Precedence: `Telemetry::resolveClientGeoUsing()` hook → Cloudflare `CF-IPCountry` → MaxMind database. Applies to server page views and the browser ingest endpoint |
