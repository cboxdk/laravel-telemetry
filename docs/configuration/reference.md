...
| `instrument.http_ignore_own_routes` | `TELEMETRY_HTTP_IGNORE_OWN_ROUTES` | `true` — this package's own routes (Prometheus scrape endpoints, browser span ingest, RUM asset, source map upload) are left uninstrumented, at whatever path each is configured with. A 15s scrape is ~5,700 requests a day measuring nothing about the app; Prometheus already times its own scrapes. Set `false` to instrument them like any other route |
...
| `instrument.native_screens` | `TELEMETRY_INSTRUMENT_NATIVE_SCREENS` | `true` — NativePHP for Mobile v4 screens, in two halves. **Automatic:** `screen.view` events, `screen.views` counter and `screen.view.duration` histogram, driven by the upstream `Native\Mobile\Events\Screen\*` lifecycle events on `nativephp/mobile` ^4.1, behind a `class_exists` guard — 4.0.x never arms them. **Opt-in:** `screen.interaction`/`screen.native_event` spans carrying `screen.name` + `screen.event.type`, plus `screen.interaction.duration{screen,type}` and `screen.interactions.failed`, which need the app to forward `dispatch()`/`dispatchNativeEvent()` from a `NativeComponent` base class — upstream announces no interaction, and that is exactly where the flush has to happen because the runloop holds one request open for the whole screen. See the [NativePHP cookbook](../cookbook/nativephp.md) |
...
| `instrument.filesystem` | `TELEMETRY_INSTRUMENT_FILESYSTEM` | `true` — `storage.operations{disk,operation}` counter + a `storage {operation}` detail span per disk operation (put, get, delete, copy, move, …), driver-agnostic (local, S3, whatever Flysystem supports) via a `Factory`/`Filesystem` decorator. Instruments both `Storage::disk('x')->put(...)` and the `Storage::put(...)` default-disk shorthand. Paths are safe on spans (per-occurrence) but never metric labels — same rule as query text. An instrumented disk is a real `Illuminate\Filesystem\FilesystemAdapter`, so code that type-hints the concrete adapter rather than the contract (Statamic's imaging, for one) keeps working |
| `instrument.filesystem_ignore_disks` | — | `[]` — disks left entirely uninstrumented, by name. An instrumented disk satisfies `instanceof FilesystemAdapter`, but PHP cannot choose a parent class at runtime, so it can never BE an adapter *subclass* (`AwsS3V3Adapter`, or a package's own). List a disk here if something type-hints the exact subclass and breaks |
